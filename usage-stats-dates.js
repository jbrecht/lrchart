...
var dataTables = {};
var services = ['submitter', 'payload_schema', 'curator','keyword'];//, 'keyword'];
...
function buildDateBins(chartRows, service) {
	...
	var form = "yyyy-MM-dd";
	var today = $.date();
	dateBins[0] = today.format(form);
	dateBins[1] = today.adjust("D", - 30).format(form);
	...
	function callDateStats(service, startKey, endKey, term, index) {
		...
		var callObj = {
			url : NODE_URL + '/resource_data/_design/lrstats-' + service + '/_view/docs?startkey='+startKey+'&endkey='+endKey,
			dataType : 'jsonp',
			jsonp : 'callback'
		};		
